...
function CharityCard({ cover, avatar, title, ein }: CharityCardProps) {
  ...
  const addCharityToFavorite = async (ein: string) => {
    ...
    if (userData.favorites?.includes(ein)) {
      ...
      let newList = userData.favorites.filter((favorite)=>favorite!==ein)
      let response = await updateDocument({favorites:newList})
...
if(!userData.favorites){
  ...
  const updateDocument=async (data)=>{
    const docRef = await setDoc(doc(db,"users",auth.currentUser.uid),data ).catch((err)=>{
  ...
  return (
    <div>
      <Card
        ...
        cover={<img alt={title} src={cover} style={{width:"300px",height:"200px"}} />}
        actions={[
          <HeartOutlined
